    ...
    <>
      ...
      <nav id="nav" className={scrolled ? 'scrolled' : ''}>
        ...
        <ul className={`nav-links${mobileMenuOpen ? ' open' : ''}`}>
          {['Features', 'Platforms', 'KPIs', 'Chat', 'Pricing'].map((l) => (
            <li key={l}><a href={`#${l.toLowerCase()}`} onClick={() => setMobileMenuOpen(false)}>{l}</a></li>
          ))}
        ...
        <div className="hero-inner">
          <div className="hero-left">
            ...
            <p className="hero-sub">
              Zofi connects to your Meta, Google, and ad accounts. She analyses every campaign, spots wasted budget, and fixes your ads — just by having a conversation with her.<br /><br />
      ...
      <div className="marquee-section">
        <div className="marquee-inner" id="marquee">
          {Array(2).fill(['Meta Ads', 'Google Ads', 'Ads', 'Facebook Pixel', 'Google Analytics', 'Google Tag Manager', 'Shopify', 'WooCommerce', 'Pixel', 'Meta Business Suite']).flat().map((item, i) => (
      ...
      <section className="problem" id="features">
        <div className="section-inner">
          ...
          <div className="problem-grid">
            {[
              { delay: 'reveal-delay-1', icon: '😤', title: "You're flying blind", body: "You're spending money on Meta, Google, and but have no idea which platform is actually working. The dashboards don't talk to each other." },
      ...
      <section className="kpi-section" id="kpis">
        <div className="section-inner">
          ...
          <div className="kpi-unique reveal">
            {[
              { icon: '🔀', title: 'Cross-Platform Attribution', body: 'Track customers who saw a ad, searched Google, then bought from a Meta retargeting ad. Know the full journey, not just the last click.' },
      ...
      <style>{`
        * { box-sizing: border-box; }
        ...
        .platform-tabs { display: flex; gap: 8px; margin: 40px 0 32px; flex-wrap: wrap; }
        ...
        .cs-avatar { width: 40px; height: 40px; border-radius: 50%; background: var(--violet); display: flex; align-items: center; justify-content: center; color: white; font-weight: 800; font-size: 16px; flex-shrink: 0; }
        .cs-info .cs-name { font-size: 15px; font-weight: 700; color: white; }
        .cs-info .cs-role { font-size: 12px; color: rgba(255,255,255,0.45); }
        .cs-online { display: flex; align-items: center; gap: 5px; margin-left: auto; font-size: 12px; color: var(--mint); white-space: nowrap; }
        ...
        .cs-input { flex: 1; padding: 12px 16px; border-radius: 10px; border: 1.5px solid var(--gray200); font-family: 'DM Sans', sans-serif; font-size: 14px; color: var(--gray800); outline: none; transition: border-color 0.2s; min-width: 0; }
        .cs-input:focus { border-color: var(--violet); }
        .cs-send { width: 42px; height: 42px; border-radius: 10px; background: var(--violet); border: none; color: white; font-size: 16px; cursor: pointer; transition: all 0.2s; display: flex; align-items: center; justify-content: center; flex-shrink: 0; }
        ...
        .toggle-track { width: 48px; height: 26px; background: var(--violet); border-radius: 100px; cursor: pointer; position: relative; transition: background 0.2s; flex-shrink: 0; }
        ...
        .price-features { list-style: none; display: flex; flex-direction: column; gap: 10px; padding: 0; margin: 0; }
        ...
        .trust-row { display: flex; align-items: center; justify-content: center; gap: 28px; margin-top: 28px; flex-wrap: wrap; }
        ...
        @media (max-width: 1024px) {
          ...
          .dashboard-wrap { animation: fadeUp 0.8s ease 0.3s both; max-width: 560px; margin: 0 auto; }

          section { padding: 72px 24px; }
          .kpi-section { padding: 72px 24px; }
          .final-cta { padding: 80px 24px; }
          footer { padding: 48px 24px 24px; }

          .problem-grid { grid-template-columns: repeat(2, 1fr); }

          .platform-panel.active { grid-template-columns: 1fr; gap: 28px; }

          .kpi-metrics { grid-template-columns: repeat(3, 1fr); }

          .kpi-unique { grid-template-columns: repeat(2, 1fr); }

          .pricing-grid { grid-template-columns: 1fr; max-width: 480px; gap: 24px; }
        ...
        @media (max-width: 768px) {
          ...
          .nav-links a { display: block; padding: 14px 0; font-size: 16px; color: rgba(255,255,255,0.8); }
          .nav-links a:hover { color: white; }
          ...
          .dashboard-wrap { animation: fadeUp 0.8s ease 0.3s both; }

          /* Sections */
          section { padding: 56px 20px; }
          .kpi-section { padding: 56px 20px; }
          .final-cta { padding: 64px 20px; }
          footer { padding: 40px 20px 24px; }

          .section-headline { font-size: 30px; letter-spacing: -1px; }
          .section-sub { font-size: 15px; }
          .final-cta .section-headline { font-size: 32px !important; }

          /* Problem */
          .problem-grid { grid-template-columns: 1fr; margin-top: 36px; }
          .problem-strip { font-size: 15px; padding: 20px; }
          .problem-strip strong { font-size: 17px; }

          /* Platforms */
          .platform-tabs { gap: 6px; }
          .ptab { padding: 8px 14px; font-size: 13px; }
          .mock-row { grid-template-columns: 2fr 1fr 1fr; font-size: 12px; padding: 10px 14px; }
          .mock-row > div:nth-child(4) { display: none; }
          .mock-row.head > div:nth-child(4) { display: none; }

          /* KPI */
          .kpi-metrics { grid-template-columns: repeat(2, 1fr); margin: 32px 0 28px; }
          .kpi-num { font-size: 22px; }
          .kpi-card { padding: 16px 14px; }
          .kpi-unique { grid-template-columns: 1fr; gap: 12px; }

          /* Chat */
          .cs-body { padding: 16px; max-height: 360px; }
          .cs-bubble { max-width: 85%; font-size: 13px; padding: 10px 14px; }
          .cs-suggestions { padding: 12px 16px; }
          .cs-input-row { padding: 12px 16px; }
          .cs-chip { font-size: 12px; padding: 6px 10px; }
        ...
        @media (max-width: 400px) {
          ...
          .hero-badge { flex-wrap: wrap; text-align: center; justify-content: center; }
          .dash-metrics { grid-template-columns: 1fr 1fr; }
          .dash-metrics .dash-metric:last-child { grid-column: 1 / -1; }
        }
      `}</style>
    </>
  )
