...
const META_TOOLS = [
  ...
  },
  {
    ...
    description: "Create a new Meta campaign (starts PAUSED).",
    input_schema: {
      type: "object",
      properties: {
        name: { type: "string" },
  ...
  },
  {
    ...
    description: "Create a Meta ad set. Check campaign bid_strategy first. For CBO campaigns, omit daily_budget_cents.",
    input_schema: {
      type: "object",
      properties: {
  ...
  },
  {
    ...
    description: "Upload an image to the Meta ad account image library. Returns image_hash. Call BEFORE meta_create_ad when user provides an image.",
    input_schema: {
      type: "object",
      properties: {
  ...
  {
    name: "meta_create_ad",
    description: "Create a Meta ad (image or text/link). WITH image: provide image_hash. WITHOUT image: omit image_hash.",
    input_schema: {
      type: "object",
      properties: {
        adset_id: { type: "string" }, name: { type: "string" }, page_id: { type: "string" },
        image_hash: { type: "string" }, headline: { type: "string" }, body: { type: "string" },
        link_url: { type: "string" },
        call_to_action: { type: "string", enum: ["LEARN_MORE","SHOP_NOW","SIGN_UP","DOWNLOAD","CONTACT_US","APPLY_NOW","GET_OFFER","ORDER_NOW","WATCH_MORE","GET_QUOTE","SUBSCRIBE","NO_BUTTON"] },
...
const GOOGLE_TOOLS = [
  ...
  },
  {
    ...
    input_schema: {
      type: "object",
      properties: {
        date_range: { type: "string", enum: ["LAST_7_DAYS","LAST_14_DAYS","LAST_30_DAYS","LAST_90_DAYS","THIS_MONTH","LAST_MONTH"] },
        campaign_ids: { type: "array", items: { type: "string" } },
      },
      required: ["date_range"],
    },
  },
  {
    ...
    input_schema: {
      ...
      properties: {
        name: { type: "string" },
        advertising_channel_type: { type: "string", enum: ["SEARCH","DISPLAY","SHOPPING","VIDEO","PERFORMANCE_MAX"] },
        daily_budget_micros: { type: "number" },
        bidding_strategy: { type: "string", enum: ["MANUAL_CPC","MAXIMIZE_CLICKS","MAXIMIZE_CONVERSIONS","TARGET_CPA","TARGET_ROAS","MAXIMIZE_CONVERSION_VALUE"] },
  ...
  },
  {
    ...
    input_schema: {
      type: "object",
      properties: { campaign_id: { type: "string" }, new_daily_budget_micros: { type: "number" }, reason: { type: "string" } },
  ...
  },
  {
    ...
    input_schema: {
      type: "object",
      properties: { campaign_id: { type: "string" }, status: { type: "string", enum: ["ENABLED","PAUSED","REMOVED"] }, reason: { type: "string" } },
      required: ["campaign_id", "status", "reason"],
    },
  },
  {
    ...
    input_schema: {
      type: "object",
      properties: { campaign_ids: { type: "array", items: { type: "string" } }, status: { type: "string", enum: ["ENABLED","PAUSED"] }, reason: { type: "string" } },
      required: ["campaign_ids", "status", "reason"],
    },
  },
  {
  ...
  },
  {
    ...
    input_schema: {
      ...
      properties: {
        ...
        ad_group_type: { type: "string", enum: ["SEARCH_STANDARD","DISPLAY_STANDARD","SHOPPING_PRODUCT_ADS"] },
      },
  ...
  },
  {
    ...
    input_schema: {
      type: "object",
      properties: {
        ad_group_id: { type: "string" }, campaign_id: { type: "string" },
  ...
  },
  {
    ...
    input_schema: {
      type: "object",
      properties: {
        ad_group_id: { type: "string" }, final_url: { type: "string" },
...
async function executeTool(name: string, input: ToolInput, creds: Credentials): Promise<unknown> {
  ...
  if (name.startsWith("meta_")) {
    if (!creds.meta) return { error: "Meta Ads is not connected. Ask the user to connect their Meta account first." };
    ...
    if (name === "meta_bulk_update_campaign_status") {
      ...
      const results = await Promise.all(campaign_ids.map(async (id) => {
    ...
    if (name === "meta_create_adset") {
      const { campaign_id, name: n, daily_budget_cents, targeting_countries, age_min, age_max, optimization_goal, billing_event, bid_amount_cents } = input as {
        campaign_id: string; name: string; daily_budget_cents?: number; targeting_countries: string[];
        age_min?: number; age_max?: number; optimization_goal: string; billing_event: string; bid_amount_cents?: number;
      };
      const targeting: Record<string, unknown> = { geo_locations: { countries: targeting_countries }, targeting_automation: { advantage_audience: 0 } };
      ...
      const p = new URLSearchParams({ campaign_id: String(campaign_id), name: n, targeting: JSON.stringify(targeting), optimization_goal, billing_event: effectiveBilling, status: "PAUSED", access_token: tok });
      if (daily_budget_cents) p.set("daily_budget", String(Math.round(daily_budget_cents)));
      ...
      if (!result?.id) {
        ...
        const isBudgetConflict = String(err?.error_subcode) === "1885621" || String(err?.error_user_msg ?? "").toLowerCase().includes("budget");
        if (isBudgetConflict) {
          const p2 = new URLSearchParams({ campaign_id: String(campaign_id), name: n, targeting: JSON.stringify(targeting), optimization_goal, billing_event: effectiveBilling, status: "PAUSED", access_token: tok });
    ...
    if (name === "meta_create_ad") {
      ...
      const linkData: Record<string, unknown> = { link: link_url, message: body, name: headline, call_to_action: { type: call_to_action } };
  ...
  if (name.startsWith("google_")) {
    if (!creds.google) return { error: "Google Ads is not connected. Ask the user to connect their Google account first." };
...
export async function POST(request: NextRequest) {
  ...
  // Build tools based on connected platforms
